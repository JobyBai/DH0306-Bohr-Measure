 ...
 * @brief 初始化模块
 */
void SineAnalyzer_Init(TIM_HandleTypeDef *htim_ic, ADC_HandleTypeDef *hadc,
		DMA_HandleTypeDef *hdma, uint16_t *adc_buf) {
	s_htim_ic = htim_ic;
	s_hadc = hadc;
	s_hdma = hdma;
	s_adc_buf = adc_buf;
...
void SineAnalyzer_Process(void) {
	...
	s_new_data_ready = 0; // 清除标志，防止重复处理
	 ...
	 uint16_t half_period_points = (uint16_t) (s_result.period_sec
	 		* SA_SAMPLING_RATE_HZ / 2.0f);

	 // 边界保护：防止点数超出缓冲区或过小
	 if (half_period_points > SA_ADC_BUF_SIZE)
	 	half_period_points = SA_ADC_BUF_SIZE;
	 if (half_period_points < 10)
	 	half_period_points = 10; // 至少需要少量点才能计算
	 ...
	 if (start_pos >= 0) {
	 	...
	 	memcpy(&temp_buf[0], &s_adc_buf[SA_ADC_BUF_SIZE - tail_len],
	 ...
	 // 计算相位差
	 s_result.phase_diff_1 = Encoder_Delta(rising_edge_count, first_code_num)
	 		/ s_result.period_sec;
	 s_result.phase_diff_2 = Encoder_Delta(falling_edge_count - 2000,
	 		first_code_num) / s_result.period_sec;
